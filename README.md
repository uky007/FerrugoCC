...
`--fobfuscate` フラグで最適化の代わりに5つの難読化パスを TACKY IR 上で適用する。
...
- **Pass 3 — 不透明述語（Opaque Predicates）**: 5回に1回、値生成命令を常に真の条件分岐で囲む
  ...
  if ((42 * 43) % 2 == 0) { /* 本物の命令 */ } else { /* 到達不能な偽コード */ }
  ```
- **Pass 4 — 制御フロー平坦化（CFF）**: 関数内の基本ブロックを state 変数 + dispatch ループに変換
  ```c
  // 元の if-else-for 等の構造を隠蔽
  state = 0;
  dispatch: switch(state) { case 0: ...; state = 1; goto dispatch; ... }
  ```
